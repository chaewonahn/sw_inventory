...
jQuery.getJSON(GetQueryStringParams("config","lucky_config.json"), function(data, textStatus, jqXHR) {
	config=data;
...
function initSigma(config) {
	...
	if (config.sigma && config.sigma.mouseProperties)
		...
		mouseProps={
        minRatio: 0.5, // How far can we zoom out?
    ...
    sigInst.position(250,100,0.65).draw();
/*    function lookupNodesByKeyValue(a, key, value) {
      return a.graph.nodes().filter(node => node[key] === value);
    }
    function lookupNodeByKeyValue(a, key, value) {
      return lookupNodesByKeyValue(a, key, value).pop();
    }
    function lookupNodeById(a, value) {
      return lookupNodeByKeyValue(a, 'ID', value);
    }
    var key = 3866;
    console.log("Node 'n2' => " + lookupNodeById(sigInst, "3866").label);*/
	}
...
console.log(sigInst.clusters);
    ...
    var a = [],
        b,x=1;
		for (b in sigInst.clusters) a.push('<div style="line-height:12px"><a href="#' + b + '"><div style="width:40px;height:12px;background:' + b + ';display:inline-block"></div> Group ' + (x++) + ' (' + sigInst.clusters[b].length + ')</a></div>');
    //a.sort();
    ...
    $("#zoom").find("div.z").each(function () {
        ...
        a.click(function () {
			if (b == "center") {
				sigInst.position(250,150,0.65).draw();
			} else {
		        var a = sigInst._core;
...
function Search(a) {
    ...
    this.search = function (a) {
        ...
        this.lastSearch = a;
        this.results.empty();
        if (1 >= a.length) this.results.html("<i>최소한 한 글자 이상의 검색어를 입력하세요.</i>");
        else {
            ...
            0 == c.length && !b && a.push("<i>검색 결과가 없습니다.</i>");
...
function nodeNormal() {
    !0 != $GP.calculating && !1 != sigInst.detail &&
    (showGroups(!1), $GP.calculating = !0, sigInst.detail = !0, $GP.info.delay(400).animate({width:'hide'},350),$GP.cluster.hide(),
...
/////////////////////////////////THIS IS WHERE WE GET RID OF THE NAS
        ...
        if (image_attribute) {
          image_attribute.filter(x => x != null)
